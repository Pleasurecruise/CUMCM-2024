...
# 敏感性分析参数
sensitivity_params = {
    'price': [0.9, 1.1],  # 价格上下浮动10%
    'yield': [0.9, 1.1],  # 产量上下浮动10%
    'cost': [0.9, 1.1]    # 成本上下浮动10%
}

# 进行敏感性分析
for param, factors in sensitivity_params.items():
    for factor in factors:
        yearly_data = {2023: planting_data}  # 重置数据
        yearly_profit = {}  # 重置利润
        ...
        # 保存敏感性分析结果
        sensitivity_result_df = pd.DataFrame(yearly_profit.items(), columns=['year', 'profit'])
        sensitivity_result_df.to_excel(f'results/sensitivity_{param}_{factor}.xlsx', index=False)
